...
const freeRequest = (method, url, query) => {
...
const financeRequest = (method, url, query) => {
    const req = unirest(method, `${process.env.YAHOO_FINANCE_API}${url}`);
    if (query) {
        req.query(query);
    }
    req.headers({
...
module.exports = { freeRequest, financeRequest };
